...
function describe(name: string, text: string, s: Record<string, unknown>, images: number): string {
  ...
  switch (name) {
    ...
    case "mcp__browser__console_read": { const j = json(text) as { total?: number; shown?: number; counts?: { error?: number; warn?: number }; entries?: { level: string; text: string }[] } | null; if (!j || typeof j.total !== "number") return firstLine(text); const err = j.counts?.error ?? 0, warn = j.counts?.warn ?? 0; const last = j.entries?.filter((e) => e.level === "error").at(-1); return `${j.total} entr${j.total === 1 ? "y" : "ies"}${err ? ` · ${err} error${err === 1 ? "" : "s"}` : ""}${warn ? ` · ${warn} warning${warn === 1 ? "" : "s"}` : ""}${last ? ` · ${clip(last.text.split("\n")[0], 50)}` : ""}`; }
    case "mcp__browser__network_read": { const j = json(text) as { total?: number; failed?: number; entries?: { url: string; status: number | null; method: string; ok: boolean | null }[] } | null; if (!j || typeof j.total !== "number") return firstLine(text); const bad = j.entries?.filter((e) => e.ok === false).at(-1); return `${j.total} request${j.total === 1 ? "" : "s"}${j.failed ? ` · ${j.failed} failed` : ""}${bad ? ` · ${bad.status || "✗"} ${bad.method} ${clip(bad.url.replace(/^https?:\/\/[^/]+/, ""), 40)}` : ""}`; }
    case "mcp__browser__type": { const j = json(text) as { typed?: number; trusted?: boolean; tag?: string } | null; return j && typeof j.typed === "number" ? `typed ${j.typed} char${j.typed === 1 ? "" : "s"}${j.tag ? ` into ${j.tag}` : ""}${j.trusted === false ? " · synthetic" : ""}` : firstLine(text); }
